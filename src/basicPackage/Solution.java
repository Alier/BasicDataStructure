...
public class Solution {
	...
	/*
	 * Best Time to Buy and Sell Stock III Say you have an array for which the
	...
	/*
	 * will make a int[][] with buying day as row, selling day as column.
	 * Calculate the biggest sum of any two qualified pair. e.g: one is
	 * int[row1][column1], second is int[row2][column2] , then these two have to
	 * fulfill the following to be a qualified pair: row1 < column 1 < row2 <
	 * column2 , which means: first one have to buy first then sell, and the
	 * sell date has to be earlier than buy date for second transaction
	 */
	public int maxProfit3(int[] prices) {
		...
		/*
		 * Index of prices are the date: buy could only happen between index 0
		 * to (prices.length-2); sell could only happen between index 1 to
		 * (prices.lengh-1);
		 */
		// form the matrix
		// each arraylist in the posTransByBuyDate would be all pos transactions
		...
		for (int i = 0; i < prices.length - 1; i++) {
			...
			for (int j = i + 1; j < prices.length; j++) {
				...
				// if it's positive, put in the arrayList as candidates to
				// choose from
				if (profits[i][j] > 0) {
		...
		for (int i = 0; i < posTransByBuyDate.size() - 1; i++) {
			...
			if (transOfFirstDay.size() == 0)
				...
				for (int j = 0; j < transOfFirstDay.size(); j++) {
					Transaction firstTran = transOfFirstDay.get(j); // specific
																	// transaction
					int firstTranSellDate = firstTran.sellDate;
					for (int k = firstTranSellDate + 1; k < posTransByBuyDate
							.size(); k++) {
						ArrayList<Transaction> transOfSecondDay = posTransByBuyDate
								.get(k);
						int newSum = firstTran.profit
								+ getMaxProfit(transOfSecondDay);
						if (newSum > maxSum)
							maxSum = newSum;
					}
				}
	...
	/*
	 * utility function, return the biggest integer from this square matrix,
	 * only looking in: row starting from StartRow, column has to be bigger than
	 * the row number.
	 * 
	 * e.g, for matric 1 2 3 4 4 5 6 7 7 8 9 8 3 2 4 1
	 * 
	...
	public int maxProfit2(int[] prices) {
		...
		// go through profits array, record all index for positive profit in one
		// day turn-over transactions
		...
		/*
		 * this profit array should have positive and negative numbers. e.g
		 * [-1,2,-3,4,5,6,-7,8] from head, merge positive nodes in sequence that
		 * could be merged(indicating merging the two transactions into one)
		 * merged new node should have value > both positive nodes, otherwise
		 * don't merge. Put the new node in a new ArrayList.
		 */
		...
		// after second merge, find the biggest two in the profits. Index in
		// posIndex.
		if (posIndex.size() == 1)
			return profits[posIndex.get(0)];
	...
	public int maxProfit1(int[] prices) {
		...
		for (int midIndex = 0; midIndex < prices.length; midIndex++) {
			...
			int rightPartProfit = maxProfitRange(midIndex + 1,
	...
	public ArrayList<ArrayList<Integer>> permuteUnique(int[] num) {
		...
		for (int i = 0; i < num.length; i++) {
			...
			if (!visited.contains(curNode)) {
				...
				// merge the restPermutes with curNumber to create curNumber's
				// permute
